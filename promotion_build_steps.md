...
| Gates (Étape 1, blocking unless noted) — **automated** | (1) final-period grades computed for every classroom [blocking]; (2) bulletins générés [advisory]; (3) exam results recorded for every cohort student [blocking **always**, regardless of that exam's passing mode]; (4) Résumé des effectifs PDF downloaded in-flow + "envoyez-le à ScolaDesk" prompt, timestamp in `app_settings` [blocking]; (5) successful sync from today [blocking, reuses `hasSuccessfulFullSync()` from Phase 7]. | Prevents promoting on stale/incomplete data — these are DB-verified, not self-reported. **Correction (owner, 2026-07-11):** entry was originally skipped when a level's mode was `moyenne_only` — owner wants results always manually entered and verified regardless of whether the mode uses them in the verdict; no automatic bypass. |
...
| Verdict — non-cohort levels | `moy_cumulative ≥ passage_cutoff` (app_settings key, admin-editable via Paramètres, defaults `'10'`). **Correction (Step 3 build):** the design originally named `promotion_pass_average` — that key is seeded but dead/unused everywhere in the code; `passage_cutoff` is the real, live setting already driving `passage_admis` on the report card. Using it keeps the promotion verdict consistent with what's printed on the bulletin. | Existing setting, no new config needed — corrected to the one actually wired up. |
...
- Étape 2: exam-results entry grid, one tab per cohort exam type — always shown for every cohort level regardless of its passing mode (mode only decides whether the score feeds the verdict, never whether it's collected).
...
3. Set one exam type to `exam_only`, one to `both`, leave one `moyenne_only` → confirm Étape 2 shows entry grids for all three (entry is always required), and verdicts respect each one's configured mode (the `moyenne_only` exam's score is recorded but ignored by the verdict calc). Toggle a level's `is_exam_cohort` on/off in the new Paramètres page and confirm it's reflected in Étape 1/2.
